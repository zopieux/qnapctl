...
bool LCD::open(const QString &portName) {
  if (serial_port_) serial_port_->deleteLater();
...
bool LCD::write(int line, const QByteArray &chars) {
  ...
  if (!serial_port_) return false;

  if (!(0 <= line && line <= 1)) return false;
...
void LCD::readyRead() {
  ...
  if (serial_port_->bytesAvailable() < MSG_SIZE) return;

  char buf[MSG_SIZE];
  if (serial_port_->read(buf, MSG_SIZE) != MSG_SIZE) return;

  if (qstrncmp(buf, kButtonPressHeader, 3) != 0) return;
  ...
  if (has_state_ && btn_enter_state != btn_enter_state_) {
    emit buttonEvent(Daemon::PanelButton::ENTER, btn_enter_state);
  }

  if (has_state_ && btn_select_state != btn_select_state_) {
    emit buttonEvent(Daemon::PanelButton::SELECT, btn_select_state);
  }
